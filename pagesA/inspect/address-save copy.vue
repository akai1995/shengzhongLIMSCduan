  ...
  <view>
	...
	<view class="toContent">
      ...
      <view class="ut-view">
        ...
        <view
          class="tag"
          :class="copy.length > 0 ? 'back1' : 'back2'"
          @click="pasteContent()"
          >识别</view
        >
      </view>
      ...
      <view class="from m-t2">
        <u--form :model="form" labelWidth="150rpx" ref="uForm">
          ...
          <u-form-item label="省市区" prop="city" @click="showNativeSelect = true">
            ...
            <address-picker
              :show="showNativeSelect"
              @confirm="confirmAddress"
              @cancel="showNativeSelect = false"
              :addressData="defaultNative"
              :closeOnClickOverlay="true"
              @close="showNativeSelect = false"
            ></address-picker>
          </u-form-item>
          ...
          <u-form-item label="">
            <u-radio-group v-model="checkbox">
    ...
    submit() {
      this.$refs.uForm.validate().then((res) => {
        ...
        if (this.type == 0) {
          ...
          updateInspectionAddress(this.form).then((res) => {
            if ((res.code = 200)) {
              uni.navigateBack();
            } else {
              uni.$u.toast("添加失败");
