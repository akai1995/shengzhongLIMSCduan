...
import { WKSDK, Message, Channel, MessageStatus, MessageExtra, Conversation } from "wukongimjssdk";
import BigNumber from "bignumber.js";
export class Convert {
	static toConversation(convMap : any) : Conversation {
		const channel = new Channel(convMap['channel_id'], convMap['channel_type'])
		const conversation = WKSDK.shared().conversationManager.createEmptyConversation(channel);
		conversation.unread = convMap['unread']
		conversation.timestamp = convMap['timestamp']
		const recents = convMap['recents']
	...
	static toMessage(msgMap : any) : Message {
		...
		const contentObj = msgMap['content']
		let contentType = 0
		if (contentObj) {
