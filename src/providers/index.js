...
export const getAddressByCode = (code, splitStr = '-') => {
	...
	const pC = code.slice(0, 2) + '0000'; const cC = code.slice(0, 4) + '00'; const qC = code.slice(0, 6)
 ...
 * @param content 提示的标题
 */
export const onToast = (content) => { uni.showToast({ title: content, icon: 'none', }) }

/**
 * 显示模态弹窗
 * @param content 提示的标题
 */
export const onShowConfirm = (content) => { return new Promise((resolve, reject) => { uni.showModal({ title: '提示', content: content, cancelText: '取消', confirmText: '确定', success: (res) => resolve(res), fail: (err) => reject(err) }) }) }

export const tansParams = (params) =>  {
	let result = ''
	for (const propName of Object.keys(params)) {
		const value = params[propName]; var part = encodeURIComponent(propName) + '='
		if (value !== null && value !== '' && typeof(value) !== 'undefined') {
			if (typeof value === 'object') {
				for (const key of Object.keys(value)) {
					if (value[key] !== null && value[key] !== '' && typeof(value[key]) !== 'undefined') {
						const params = propName + '[' + key + ']'; var subPart = encodeURIComponent(params) + '='
...
export const guid = () => { return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) { var r = (Math.random() * 16) | 0, v = c == 'x' ? r : (r & 0x3) | 0x8; return v.toString(16); }); }
