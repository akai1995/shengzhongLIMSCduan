...
<view> <!-- 建议template下只有一个根节点（最外面套一层view），如果不小心踩到了vue3的Fragments(multi-root 多个根节点)特性（vue2编译会报错，vue3不会），可能会出现奇奇怪怪的兼容性问题 -->
	<view style="padding:5px 10px 0">
		...
		<navigator :url="pageNewPath" style="display: inline;"><button size="mini" type="default">新开页面({{pageDeep}})</button></navigator>		
		<navigator url="page_i18n" style="display: inline;"><button size="mini" type="default">国际化多语言</button></navigator>		
		<navigator url="page_asr" style="display: inline;"><button size="mini" type="default">asr语音识别</button></navigator>		
<!-- #ifdef APP || H5 -->
		<navigator url="page_nvue" style="display: inline;"><button size="mini" type="default">nvue原生页面</button></navigator>
		<navigator url="page_renderjsOnly" style="display: inline;"><button size="mini" type="default">纯renderjs调用</button></navigator>
<!-- #endif -->		
	...
	<!-- 录音格式选择 -->
	<view style="padding:10px 10px 0">类型：
	...
	<view style="padding:5px 0 0 10px">
		...
		<view>
			...
			<view class="recwave" :style="{display:recwaveChoiceKey!='SurferView'?'none':''}"><canvas type="2d" class="recwave-SurferView"></canvas><canvas type="2d" class="recwave-SurferView-2x" style="width:600px;display:none"></canvas></view>
			<view class="recwave" :style="{display:recwaveChoiceKey!='Histogram1'?'none':''}"><canvas type="2d" class="recwave-Histogram1"></canvas></view>
			<view class="recwave" :style="{display:recwaveChoiceKey!='Histogram2'?'none':''}"><canvas type="2d" class="recwave-Histogram2"></canvas></view>
			<view class="recwave" :style="{display:recwaveChoiceKey!='Histogram3'?'none':''}"><canvas type="2d" class="recwave-Histogram3"></canvas></view>
		</view>
	...
	<!-- 手撸播放器 -->
	<view style="padding-top:10px"><TestPlayer ref="player" /></view>
	
	<!-- 上传、语音通话等功能 -->
	<view v-show="showUpload"><TestUploadView /><TestRtVoiceView /></view>
	...
	<view style="padding:10px 10px">
		<view>
			<navigator url="page_vue3____composition_api" style="display: inline;"><button size="mini" type="default">vue3组合式api页面</button></navigator>			
			<button size="mini" type="default" @click="loadVConsole">显示vConsole</button>
		</view>
<!-- #ifdef APP -->
		<view>
			<navigator url="page_renderjsWithout" style="display: inline;"><button size="mini" type="default">逻辑层编码UniWithoutAppRenderjs后台录音更稳</button></navigator>
		</view>
		<view>
...
<!-- #endif -->
		...
		<view>
			<view v-for="(item,idx) in testMsgs" :key="`idx_${idx}`" style="border-top:1px dashed #eee; padding:5px 0" :style="{color:item.color==1?'red':item.color==2?'green':item.color}">
				{{item.msg}}
			</view>
		</view>
	</view>
	...
	<!-- 文档地址提示 -->
	<view v-if="false" style="height:10px;background:#eee"></view>
	<view v-if="false" style="padding:10px 10px">
	...
	<!-- 组件说明 -->
	<view v-if="false" style="margin-bottom:5px;height:10px;background:#eee"></view>
	<view v-if="false" style="padding:0 10px">
...
<!-- #ifdef APP || H5 -->
	...
	<view style="padding:10px 10px">
		<view><text style="font-size:17px;font-weight: bold;color:#f60">在逻辑层中执行代码</text><button size="mini" type="default" @click="evalExecClick">执行</button></view>
