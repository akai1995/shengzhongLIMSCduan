                    ...
                    success: (res) => {
                        if (res.confirm) { onDownLoadUpdate(updateManager) }
                            ...
                            uni.showModal({
                                ...
                                title: '温馨提示~', showCancel: false, confirmText: "确定更新",
...
const requestInterceptor = () => {
  uni.addInterceptor('request', {
    invoke(args) { args.url = args?.url || '' },
    success(args) {
        if (args.statusCode === 400) {
            uni.$emit('z-paging-error-emit', args.data);
            uni.hideLoading(); uni.hideNavigationBarLoading();
        }
    },
    fail(error) {
...
const initData = (options) => {
    if (store&&store.dispatch) {
        ...
        store.dispatch(StoreConfigs.vuex.userModule.actions.updateCurrentUserAction, (res)=>{ console.log('initData updateCurrentUserAction', res) });
        store.dispatch(StoreConfigs.vuex.userModule.actions.updatePermissionAction, (res)=>{ console.log('initData updatePermissionAction', res) });
    }
};
...
const AppCoreModule = {
	...
	onLaunch: (options) => {
		...
		window.copyCode = (codeId) => { console.log('codeId', codeId);
			const clipboard = new Clipboard(`#${codeId}`, { text(trigger) { console.log(trigger); return trigger.outerText.replace(/复制\n/g, '\n'); } });
            clipboard.on('success', (_event) => { uni.showToast({ title: '复制成功', icon: 'success' }); clipboard.destroy() });
			clipboard.on('error', (_event) => { uni.showToast({ title: '复制失败', icon: 'error' }); clipboard.destroy() });
		};
		// #endif
		requestInterceptor(); setTimeout(() => { initData(options); }, 5000);
	},
