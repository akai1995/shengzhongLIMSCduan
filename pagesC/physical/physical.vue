<template>
	<view class="wrapper">
		<u-navbar title="物流路线" :placeholder="true" border>
		...
		<view class="sub-top">
			...
			<u-tag @click="makePhone(sendOrder.driverTel)" text="联系司机" color="#5178FF" shape="circle" bgColor="#EEF2FF"
				plain> </u-tag>
		</view>
		<view class="map-box">
			<Lyn4ever-gaodeRoutePlanning width="100vw" :include-points="points" height="60vh" ref="maps" :scale="scale" :longitude="longitude"
				:latitude="latitude" :Config="Config" :startPoi="startPoi" :endPoi="endPoi" :wayPoi="wayPoi" />

		</view>
		<view class="view safe-area-inset-bottom">
			<!-- <view class="top">
				<view class="du">
			...
			<u-steps :current="stepsList.length" direction="column" dot>
		...
		data() {
			return {
				...
				Config: {
					...
					wayImage: "https://genepiapi.ypzlfx.com/file/genepi/2023/11/24/sxTMkWFRL5P0b0b8097e7a671bf67e7bcf293b608003_20231124132546A408.png",
					...
					if (res.code == 200) {
						...
						if (wagpoints.length > 0) {
							...
							this.$refs.maps.markers.push({
								...
								width: 20,
								label: {
									content: '温度' + this.temperature + '°C，湿度' + this.humidness +
										'%rh',
									fontSize: 14,
									bgColor: 'rgba(81,120,255,0.6)',
									color: '#fff',
									anchorX: 20,
									anchorY: -20,
									padding: 10,
							...
							const carLat = this.sendOrder.status == '4' || this.sendOrder.status == '5' ?
								endpoiArr[1] : startpoiArr[1]
							const carlog = this.sendOrder.status == '4' || this.sendOrder.status == '5' ?
								endpoiArr[0] : startpoiArr[0]
							this.$refs.maps.markers.push({
								height: 40,
								iconPath: this.Config.wayImage,
								id: 999,
